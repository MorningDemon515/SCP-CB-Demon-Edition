...
Function CreateRoom.Rooms(level, roomshape%, x#, y#, z#, name$ = "")
	...
	Local temp% = 0
	For rt.RoomTemplates = Each RoomTemplates
		If rt\Shape = roomshape Then temp=temp+rt\Commonness
	Next
	...
	For rt.RoomTemplates = Each RoomTemplates
		If rt\Shape = roomshape Then
			...
			If RandomRoom > temp - rt\Commonness And RandomRoom <= temp Then
				...
				r\obj = CopyEntity(rt\obj)
				scale = (8.0 / 2048)
				ScaleEntity(r\obj, scale, scale, scale)
				PositionEntity(r\obj, x, y, z)
				EntityType(r\obj, HIT_MAP)
				EntityPickMode(r\obj, 3)
				FillRoom(r)
...
Function FillRoom(r.Rooms)
	...
	Select r\RoomTemplate\Name
		Case "lockroom"
			...
			Local em.Emitters = CreateEmitter(r\x - 175.0 * RoomScale, 344.0 * RoomScale, r\z + 656.0 * RoomScale, 0)
			...
			em.Emitters = CreateEmitter(r\x - 655.0 * RoomScale, 344.0 * RoomScale, r\z + 240.0 * RoomScale, 0)
			TurnEntity(em\Obj, 90, 0, 0, True)
			EntityParent(em\Obj, r\obj)
			em\RandAngle = 3
			em\Speed = 0.035
	...
	;FlipMesh r\Objects[1]
		...
		Case "room2testroom2"
			...
			it = CreateItem("Mobile Task Force Epsilon-11", "paper", r\x - 316.0 * RoomScale, r\y + 272.0 * RoomScale, r\z + 176.0 * RoomScale)
			EntityParent(it\obj, r\obj)			
		Case "room3tunnel"
		...
		Case "room2nuke"
			...
			d\AutoClose = False : d\open = False
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), r\z - 284.0 * RoomScale,True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), r\z - 284.0 * RoomScale,True)			
			...
			For n% = 0 To 1
				r\Objects[n * 2] = LoadMesh("GFX\map\leverbase.x")
				r\Objects[n * 2 + 1] = LoadMesh("GFX\map\leverhandle.x")
		...
		Case "room2tunnel"
			...
			it = CreateItem("First Aid Kit", "firstaid", r\x + 2848.0 * RoomScale, r\y -2368.0 * RoomScale, r\z - 4568.0 * RoomScale)
			EntityParent(it\obj, r\obj) : RotateEntity(it\obj, 0, 180, 0)
			
		Case "tunnel"
			sc.SecurityCams = CreateSecurityCam(r\x, r\y + 448 * RoomScale, r\z, r)
			sc\FollowPlayer = True
			
		Case "tunnel2"
			r\Objects[0] = CreatePivot()
			PositionEntity(r\Objects[0], r\x, 544.0 * RoomScale, r\z + 512.0 * RoomScale, 0)
			EntityParent(r\Objects[0], r\obj)			
			
			r\Objects[1] = CreatePivot()
			PositionEntity(r\Objects[1], r\x, 544.0 * RoomScale, r\z - 512.0 * RoomScale, 0)
			EntityParent(r\Objects[1], r\obj)					
		...
		Case "room2closets"
			it = CreateItem("Document SCP-173", "paper", r\x + 692.0 * RoomScale, r\y + 172.0 * RoomScale, r\z + 463.0 * RoomScale)
			EntityParent(it\obj, r\obj)
			
			it = CreateItem("Gas Mask", "gasmask", r\x + 698.0 * RoomScale, r\y + 236.0 * RoomScale, r\z + 623.0 * RoomScale)
			ScaleEntity(it\obj, 0.02, 0.02, 0.02) : EntityParent(it\obj, r\obj)
			
			it = CreateItem("9V Battery", "bat", r\x + 703.0 * RoomScale, r\y + 184.0 * RoomScale, r\z - 501.0 * RoomScale)
			EntityParent(it\obj, r\obj)
			If Rand(2) = 1 Then
				it = CreateItem("9V Battery", "bat", r\x + 695.0 * RoomScale, r\y + 184.0 * RoomScale, r\z - 525.0 * RoomScale)
				EntityParent(it\obj, r\obj)
			EndIf
			If Rand(2) = 1 Then
				it = CreateItem("9V Battery", "bat", r\x + 708.0 * RoomScale, r\y + 184.0 * RoomScale, r\z - 565.0 * RoomScale)
				EntityParent(it\obj, r\obj)
			EndIf
			...
			tex = LoadTexture("GFX\npcs\duck2.png")
			EntityTexture r\Objects[0], tex
			PositionEntity (r\Objects[0], r\x + 199.0 * RoomScale, 0, r\z + 208.0 * RoomScale)
		...
		Case "room106"
			...
			For n = 0 To 2 Step 2
				r\Objects[n] = LoadMesh("GFX\map\leverbase.x")
				r\Objects[n+1] = LoadMesh("GFX\map\leverhandle.x")				
			...
			PositionEntity (r\Objects[5], r\x + 1088.0 * RoomScale, 1096.0 * RoomScale, r\z + 1728.0 * RoomScale) 
			EntityParent r\Objects[5], r\obj
...
Function InitWayPoints(loadingstart=60)
	...
	For w.WayPoints = Each WayPoints
		...
		While (w2<>Null)
			x# = Abs(EntityX(w\obj,True)-EntityX(w2\obj,True))
			If x < 6.0 Then 
				z# = Abs(EntityZ(w\obj,True)-EntityZ(w2\obj,True))
				If z < 6.0 Then 
					y# = Abs(EntityY(w\obj,True)-EntityY(w2\obj,True))
					If y < 6.0 Then 
						
						dist# = x*x+y*y+z*z;EntityDistance(w2\obj, w\obj)
						largest%=0
						If dist < 36.0 Then
							dist = Sqr(dist)
							...
							If EntityPick(pvt, 6.0)=w2\obj Then 
								found = False
								For i = 0 To 4
...
Function FindPath(n.NPCs, x#, y#, z#)
	...
	For w.WayPoints = Each WayPoints
		...
		If xtemp < 8.0 Then 
			...
			If ztemp < 8.0 Then 
				If Abs(EntityY(w\obj,True)-EntityY(temp,True)) < 8.0 Then 
	...
	Repeat 
		;DebugLog "aaaaaaaaaaaaaaaaaaaaaa"
		temp% = False
...
Function CreateMap()
	...
	For lvl = 0 To 0
		...
		x = Floor(MapWidth / 2)
		y = MapHeight - Rand(4, 7)
		
		;MapName[x, MapHeight - 2] = "start"
		
		For i = y To MapHeight - 2
		...
		Local Room1Amount%=0, Room2Amount%=0,Room2CAmount%=0,Room3Amount%=0,Room4Amount%=0
		For y = 1 To MapHeight - 1
			For x = 1 To MapWidth - 1
				If MapTemp(lvl, x, y) > 0 Then
					temp = 0
					temp = temp + Min(MapTemp(lvl, x + 1, y),1) + Min(MapTemp(lvl, x - 1, y),1)
					temp = temp + Min(MapTemp(lvl, x, y + 1),1) + Min(MapTemp(lvl, x, y - 1),1)					
					MapTemp(lvl, x, y) = temp
					If MapTemp(lvl,x,y)=1 Then Room1Amount=Room1Amount+1
					If MapTemp(lvl,x,y)=2 Then 
						If Min(MapTemp(lvl, x + 1, y),1) + Min(MapTemp(lvl, x - 1, y),1)= 2 Then
							Room2Amount=Room2Amount+1	
						ElseIf Min(MapTemp(lvl, x, y + 1),1) + Min(MapTemp(lvl, x , y - 1),1)= 2
							Room2Amount=Room2Amount+1	
						Else
							Room2CAmount = Room2CAmount+1
						EndIf
					EndIf
					If MapTemp(lvl,x,y)=3 Then Room3Amount=Room3Amount+1	
					If MapTemp(lvl,x,y)=4 Then Room4Amount=Room4Amount+1	
		...
		If temp > 0 Then
			For y = 2 To MapHeight - 2
				For x = 2 To MapWidth - 2
					If MapTemp(lvl, x, y) = 0 And Rand(7)=1 Then
		...
		DebugLog Room1Amount
		DebugLog Room2Amount
		DebugLog Room2CAmount
		DebugLog Room3Amount
		DebugLog Room4Amount
		
		MapRoom(ROOM1, Floor(0.15*Float(Room1Amount))) = "roompj"
		MapRoom(ROOM1, Floor(0.25*Float(Room1Amount))) = "914"
		MapRoom(ROOM1, Floor(0.4*Float(Room1Amount))) = "coffin"				
		MapRoom(ROOM1, Floor(0.6*Float(Room1Amount))) = "room079"	
		
		MapRoom(ROOM1, Floor(0.7*Float(Room1Amount))) = "room106"
		MapRoom(ROOM1, Room1Amount-2) = "exit1"
		MapRoom(ROOM1, Room1Amount-1) = "gateaentrance"
		MapRoom(ROOM1, 0) = "start"
		
		MapRoom(ROOM2, Floor(0.42*Float(Room2Amount))) = "room2offices2"	
		
		
		MapRoom(ROOM2, Floor(Rnd(0.1,0.15)*Float(Room2Amount))) = "room2offices"
		MapRoom(ROOM2, Floor(Rnd(0.20,0.25)*Float(Room2Amount))) = "room2testroom2"
		MapRoom(ROOM2, Floor(Rnd(0.30,0.38)*Float(Room2Amount))) = "room2sroom"	
		
		MapRoom(ROOM2, Floor(Rnd(0.45,0.5)*Float(Room2Amount))) = "room2tunnel"
		MapRoom(ROOM2, 0.55*Room2Amount) = "room2scps"
		MapRoom(ROOM2, Floor(Rnd(0.6,0.65)*Float(Room2Amount))) = "room2nuke"
		MapRoom(ROOM2, Floor(Rnd(0.8,0.86)*Float(Room2Amount))) = "testroom"
		MapRoom(ROOM2, Floor(Rnd(0.9,0.95)*Float(Room2Amount))) = "room2poffices"	
		MapRoom(ROOM2, 0) = "room2closets"
		
		MapRoom(ROOM2C, 0) = "lockroom"
		MapRoom(ROOM2C, 1) = "room2ccont"
		MapRoom(ROOM2C, 2) = "lockroom2"
		
		MapRoom(ROOM3, Floor(0.3*Float(Room3Amount))) = "room3servers"
		MapRoom(ROOM3, Floor(0.7*Float(Room3Amount))) = "room3servers2"
				...
				If MapTemp(lvl, x, y) > 0 Then
					...
					Select temp ;viereisissä ruuduissa olevien huoneiden määrä
						Case 1
							...
							EndIf
							r = CreateRoom(lvl, ROOM1, x * 8, 0, y * 8, MapName(x, y))
						...
						Case 2
							If MapTemp(lvl, x - 1, y)And MapTemp(lvl, x + 1, y) Then
								...
								EndIf
								r = CreateRoom(lvl, ROOM2, x * 8, 0, y * 8, MapName(x, y))
							...
							ElseIf MapTemp(lvl, x - 1, y) And MapTemp(lvl, x, y + 1)
								...
								EndIf
								r = CreateRoom(lvl, ROOM2C, x * 8, 0, y * 8, MapName(x, y))
								r\angle = 180
								TurnEntity(r\obj, 0, r\angle, 0)
						...
						Case 3
							...
							EndIf
							r = CreateRoom(lvl, ROOM3, x * 8, 0, y * 8, MapName(x, y))
						...
						Case 4
							...
							EndIf
							r = CreateRoom(lvl, ROOM4, x * 8, 0, y * 8, MapName(x, y))
							MapRoomID(ROOM4)=MapRoomID(ROOM4)+1
					End Select
					
					If (Floor((x + y) / 2.0) = Ceil((x + y) / 2.0)) Then
						If MapTemp(lvl, x + 1, y) Then
							CreateDoor(r\level, x * spacing + spacing / 2.0, 0, y * spacing, 90, Null, Max(Rand(-3, 1), 0))
						EndIf
						...
						If MapTemp(lvl, x, y + 1) Then
							CreateDoor(r\level, x * spacing, 0, y * spacing + spacing / 2.0, 0, Null, Max(Rand(-3, 1), 0))
						EndIf
	...
	r = CreateRoom(0, ROOM1, x * 8, 0, y * 8, "room173")
	MapRoomID(ROOM1)=MapRoomID(ROOM1)+1
	
	x = MapWidth-2
	y = MapHeight - 2
	
	r = CreateRoom(0, ROOM1, x * 8, 0, y * 8, "pocketdimension")
	MapRoomID(ROOM1)=MapRoomID(ROOM1)+1	
	
	r = CreateRoom(0, ROOM1, 0, 0, (MapHeight/2)*8, "gatea")
	MapRoomID(ROOM1)=MapRoomID(ROOM1)+1		
	
	;r = CreateRoom(0, ROOM1, (MapWidth-1)*8, 0, (MapHeight/2)*8, "forest")
	;MapRoomID(ROOM1)=MapRoomID(ROOM1)+1
...
;tarkistaa onko vieressä jotain huoneita joita varten voitaisiin tehdä opastekyltti
; 		jos on, luo kyltin ja antaa arvoksi kylttiobjektin handlen
;		jos ei, antaa arvoksi 0
Function GetSign(rx%,ry%,xdir%,ydir%)
	
	For x = Min(Max(Min(rx+xdir*10,MapWidth-1),1),rx) To Max(Max(Min(rx+xdir*10,MapWidth-1),1),rx)
		For y = Min(Max(Min(ry+ydir*10,MapHeight-1),1),ry) To Max(Max(Min(ry+ydir*10,MapHeight-1),1),ry)
			;If MapTemp(0,x,y) = 0 Then Return			
			
			roomname$ = MapName(x,y)
			...
			If texture <> "" Then
				Local obj = CopyMesh(SignObj)
				ScaleEntity obj, RoomScale, RoomScale, RoomScale, True
...
;~F#2#9#26#ED#FE#10F#12A#133#13E#149#175#184#1A8#1BD#1F7#753#77E#799#79E#7AD
;~F#7C6#848#922#93F#946#954#96D#AED
;~C#Blitz3D